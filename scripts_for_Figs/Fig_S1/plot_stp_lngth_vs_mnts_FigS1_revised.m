...
for i_mnts = 1:numel(n_mnts_stps)
    ...
    %             end
        ...
        %         err_stp_trs = mod(err_m_trs + err_drft +pi,2*pi) - pi; 
                ...
                % calculate total stepwise angular deviance for nontr and tr cases
        ...
        %         s_tr(i_dtc_err) = std(err_stp_trs);  
            ...
            equiv_N_errs_nontr_s = sqrt(dev_dtct_errs.^2/n_mnts_stp + ...
    ...
    %                (n_mnts_stp-1)*dev_mnt_errs.^2/n_mnts_stp^2 + dev_inh_err^2); % Rayl_fact* .*sin_ss_tr
            ...
            kap_eq_N_ntr = 1./equiv_N_errs_nontr_s.^2;
            kap_eq_N_tr = 1./equiv_N_errs_tr_s.^2;   
...
%          set(sc3,'MarkerEdgeColor','k') % Li'MarkerEdgeColor','k')
             ...
             plot(n_mnts_stps,smooth(stp_lngth(:,i_err)), ...
        'Color', cmaps_plot(i_err,:),'LineWidth',1.75)
...
%     colormap(cmaps)
    ...
    ylabel('Relative flight distance','FontSize',11)
    xlabel('Number maintenance steps', 'FontSize',11)
     set(gca,'YAxisLocation','Right')
     
%% new plot using lines based on substep effective error

cmaps_plot = colormap(brewermap(numel(dev_dtct_err_degs),'YlOrRd')); % '*RdYlBu')); %  colormap(parula(size(A,1))); 
...
%     plot(n_mnts_stps,bes_1_dtc(1)*ones(size(n_mnts_stps)),'--k')
for i_err = 1:numel(dev_dtct_errs)
%     sc3 = scatter(n_mnts_stps,smooth(stp_err(:,i_err)),20, ...
%         dev_dtct_err_degs(i_err)*ones(size(n_mnts_stps)),'fill');
%          set(sc3,'MarkerEdgeColor','k') % Li'MarkerEdgeColor','k')
    
    plot(n_mnts_stps,smooth(stp_err(:,i_err)), ...
        'Color', cmaps_plot(i_err,:),'LineWidth',1.75)
...
%     cb = colorbar('Location', 'NorthOutside') % ,'North'); %
%     title(cb,'Standard substep error (^o)')
%     caxis([0.5 7.5])
%     set(cb,'YTick',1:7,'TickLabels',dev_dtct_err_degs)
   
%     plot([0 115],[0 115],':k')
    axis([1 52 0 81])
    set(gca,'XTick',[1 10:10:50]) % ,'YTick',[1 10:10:60])
  
     set(gca,'YAxisLocation','Left')

    ylabel('Flight-step precision (^o)','FontSize',11)
    xlabel('Number maintenance steps', 'FontSize',11)
%      set(gca,'YAxisLocation','Left')
     
keyboard
     
    figure('Position',[700 200 FgWd FgHt]);
    cb = colorbar; % ('North');
%     title(cb,['Compass precision $ (^o)$'], ...
%         'Interpreter','latex','FontSize',9) % (cb,{'Error components (^o)'})
    title(cb,{'Compass','precision (^o)'},'FontSize',9)    
    caxis([0 60])
    colormap(cmaps)
    axis('off')
    box('off')
   ...
   colormap(cmaps_plot)
   caxis([0.5 7.5])
   set(cb,'YTick',1:7,'TickLabels',dev_dtct_err_degs,'FontSize',9)
      set(cb,'Position',[0.80 0.10 0.075 0.75]) % [left, bottom, width, height]) 
