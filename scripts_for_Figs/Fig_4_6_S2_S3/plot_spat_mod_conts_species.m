...
% n_step_maxs = [36 33 42 77 33 44 48 63 87];
% max_hs = n_step_maxs(ispc);
...
if ~plot_ms_opt
    ...
    or_plot_idx = find(or_prog == or_prog_test_sp);
    
    if numel(or_plot_idx) > 1
    ...
    map_id = [4 5 7 2 3 3 3]; % [1 1 3 6 4];
    cont_clr = cmaps(map_id(or_plot_idx),:);    
...
%     cont_clr = cmaps(map_id(or_prog_i),:);
    ...
    plot_cbr_opt = 0;
    mult_fact = min(max(round(8*80/(dep_lat_degs-lat_goal_centr)),8),20);

    cmaps = colormap(brewermap(7,'Set1')); % 6,'Dark2'));
    % point to coulours for current "solve" orien % tn prog
    map_id = [4 5 7 2 3 3 3 3 3 2]; % [1 1 3 6 4];
    cont_clr = cmaps(map_id(or_prog_i),:);    
...
%          FLatLims = [0 30];
%         
           ax = axesm ('stereo', 'Frame', 'on', 'Grid', 'off','Origin',[map_lat_cntr map_lon_cntr 0],'FlineWidth',2);
...
%     plot(Lon_degs,Lat_degs)
    ...
    lost_at_pole  = find(lat_es(i_plt_rec,:)>89.9*pi/180,1,'first');
...
%      Lat_degs = lat_es(i_plt_rec,1:lost_at_pole-1)*180/pi;
    ...
    elseif strcmp(species,'BU Rosefinch') 
        too_East = find(lon_es(i_plt_rec,:)>75*pi/180,1,'first');
        if ~isempty(too_East)
            icl_dist = min(icl_dist,too_East)*mult_fact;
        end        
    end
...
if plot_mn_opt
    ...
    is_succ = dec_close{1}<=goal_rad;
    
    all_lons = mod(spec_lon_offset+lon_es(is_succ,:)*180/pi+180,360)-180;
    all_lats = mod(spec_lon_offset+lat_es(is_succ,:)*180/pi+180,360)-180;
    
    for ii = 1:size(all_lons,2)
        
        mn_lon(ii) = 180/pi*circ_mean(pi/180*all_lons(~isnan(all_lons(:,ii)),ii));
        mn_lat(ii) = 180/pi*circ_mean(pi/180*all_lats(~isnan(all_lons(:,ii)),ii));        
...
%     h = scatterm(smooth(mn_lat),smooth(mn_lon), ...
%                 10,'fill','ro'); % ,'Mark

    h = scatterm(mn_lat,mn_lon, ...
                25,'MarkerEdgeColor',cont_clr, ...
                'MarkerFaceColor','r','Marker','s'); % ,'Mark
    
    plotm(mn_lat,mn_lon-1.5,'w'); % ,'Mark
    plotm(mn_lat,mn_lon+1.5,'w'); % ,'Mark
...
if is_trans_comp
   title([num2str(round(100*p_within_goal)) '% (T)'],'FontSize',13)
else
   title([num2str(round(100*p_within_goal)) '%'],'FontSize',13)    
end
