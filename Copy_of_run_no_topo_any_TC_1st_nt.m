...
%     step_del_Lat = nightly_del_Lat/(1+n_mnts);
    ...
    doys_pr = day(datetime(datevec(dates_0)),'dayofyear' ) -1;
                            ...
                            elseif is_sun_comp
                                ...
                                if polar_err_scale_opt
                                  ...
                                  err_dtcs(~done),max_det_err_light,max_det_err_dark);
                                      ...
                                      sun_stop(~done) = calc_sun_az(lat_es(~done,i_t),doys_pr(~done)+1);
                                      
                                end

                               if i_t > 1 % && stop_sun_reset == 1 % (mod(i_t-1,n_fls_step)==0 && stop_sun_reset == 1)

                                % first set the time leg (in this version also counts for first night of
                                % stopover
...
%                                            lon_es(~done,i_t-1)).*sin(lat_es(~done,i_t-1)); %  
                                   ...
                                   if stop_sun_reset == 1 
                                       ...
                                       % preferred direction first night of
                                       % stopover
                                       % To do this we need to include the
                                       % extra time lag and sun azimuth
                                       % shift from previous site
%                                       sun_offset(stops_over) = ...
%                                           tc_reset_only*(sun_az(stops_over) - sun_ref_az(stops_over)) ...
%                                           +  ~tc_reset_only*(dalf(stops_over,1) + ...
%                                           tc_offset(stops_over)-tc_offset_pr(stops_over) + ...
%                                            sun_stop(stops_over)-sun_az_pr(stops_over)); %
                                      sun_offset(stops_over) = sun_offset(stops_over) + ...
                                          tc_reset_only*(sun_az(stops_over) - sun_ref_az(stops_over)) ...
                                          +  ~tc_reset_only*(tc_offset(stops_over) + ...                                          
                                           sun_stop(stops_over)-sun_ref_az(stops_over)); % 
